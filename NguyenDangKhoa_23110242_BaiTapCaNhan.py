...
def and_or_search(initial_state, goal_state):
    ...
    def recursive_search(node, explored, depth_limit=1000):
...
def backtracking_search(initial_state, goal_state, max_depth=30):
    ...
    def recursive_search(node, explored, depth):
        if node.state == goal_state:
            return [node.state], [], node.cost
        ...
        for action in ["up", "down", "left", "right"]:
            ...
            if new_state:
                child_node = SearchNode(new_state, node, action, node.cost + 1)
                child_path, child_actions, child_cost = recursive_search(child_node, explored, depth + 1)

                if child_path and child_cost < best_cost:
                    best_path = [node.state] + child_path
                    best_actions = [action] + child_actions
                    best_cost = child_cost

        explored.remove(state_tuple)
        if best_path is None:
            return None, None, float('inf')
        return best_path, best_actions, best_cost
    ...
    initial_node = SearchNode(initial_state, cost=0)
    explored = set()
    path, actions, cost = recursive_search(initial_node, explored, 0)

    if path is None:
        return None, None, 0
    return path, actions, cost

def q_learning_solve(initial_state, goal_state, episodes=5000, alpha=0.1, gamma=0.9, epsilon=0.2, max_steps=30):
    from collections import defaultdict
    ...
    # Trích xuất lời giải sau huấn luyện
    state = initial_state[:]
    path = []
    actions_taken = []
    visited = set()
    for _ in range(max_steps):
        ...
        if next_state == goal_state:
            return path, actions_taken, len(path)
        state = next_state
